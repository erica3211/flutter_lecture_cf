...
class _HomeScreenState extends State<HomeScreen> {
  ...
  checkPermission() async {
    ...
    LocationPermission checkedPermisson = await Geolocator.checkPermission();

    // 한번도 권한 요청을 안한 경우
    if (checkedPermisson == LocationPermission.denied) {
      // 권한 요청
      checkedPermisson = await Geolocator.requestPermission();
    }
    if (checkedPermisson != LocationPermission.always &&
        checkedPermisson != LocationPermission.whileInUse) {
      // 권한 요청
          ...
          return Column(
            children: [
              Expanded(
                flex: 2,
                child: _GoogleMaps(
                  initialCameraLocaion: initialPosition,
...
class _GoogleMaps extends StatelessWidget {
  final CameraPosition initialCameraLocaion;
  final MapCreatedCallback onMapCreated;
  final double radius;
  final bool canChoolCheck;
  const _GoogleMaps({
    required this.initialCameraLocaion,
  ...
  Widget build(BuildContext context) {
    return GoogleMap(
      initialCameraPosition: initialCameraLocaion,
